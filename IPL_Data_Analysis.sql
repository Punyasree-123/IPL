-- How many matches were played by each player and in how many of these they batted, and how many of these they bowled

Create table cricket (
match_id int not null,ballnumber int,
inningo text,over_no int,
outcome text,
batter text,bowler text,
score int);

INSERT INTO cricket VALUES(1,1,1,0.1,'0','Mohammed Shami','Devon Conway',0),
...
from (Select batter as player, match_id from cricket
union
Select bowler as player, match_id from cricket) as A
group by player)
select c.*,coalesce(bat.total_batting,0) as batting ,coalesce(boll.total_bolling,0) as bowling
from cte as c
left join
( select batter, count(distinct match_id) as total_batting
from cricket
group by batter) as bat
on c.player=bat.batter
